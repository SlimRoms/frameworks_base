...
//                    updateCarrierLabelVisibility(false);
    ...
    private void setDisableHomeLongpress() {
        ...
        ArrayList<ActionConfig> actionConfig =
            ActionHelper.getNavBarConfig(mContext);
        ActionConfig config;
        for (int j = 0; j < actionConfig.size(); j++) {
            config = actionConfig.get(j);
            if (config.getClickAction().equals(ActionConstants.ACTION_HOME)
                    && !config.getLongpressAction().equals(ActionConstants.ACTION_NULL)) {
