...
public class CreateShortcut extends LauncherActivity {
    ...
    private boolean updateSettingType(boolean isSlim, String value) {
        // Necessary ugly code.  Do it here so we don't have to again.
        try {
            isSlim ? SlimSettings.Global.getInt(
                    getContentResolver(), value)
                    : Settings.Global.getInt(
                    getContentResolver(), value);
            mSettingType = isSlim ? SLIM_GLOBAL_INT : GLOBAL_INT;
        } catch (SettingNotFoundException p) {
            try {
                isSlim ? SlimSettings.Global.getLong(
                        getContentResolver(), value);
                        : Settings.Global.getLong(
                        getContentResolver(), value);
                mSettingType = isSlim ? SLIM_GLOBAL_LONG : GLOBAL_LONG;
            } catch (SettingNotFoundException q) {
                try {
                    isSlim ? SlimSettings.Global.getFloat(
                            getContentResolver(), value)
                            : Settings.Global.getFloat(
                            getContentResolver(), value);
                    mSettingType = isSlim ? SLIM_GLOBAL_FLOAT : GLOBAL_FLOAT;
                } catch (SettingNotFoundException r) {
                    try {
                        isSlim ? SlimSettings.System.getIntForUser(
                                getContentResolver(),
                                value, UserHandle.USER_CURRENT)
                                :Settings.System.getIntForUser(
                                getContentResolver(),
                                value, UserHandle.USER_CURRENT);
                        mSettingType = isSlim ? SLIM_SYSTEM_INT : SYSTEM_INT;
                    } catch (SettingNotFoundException a) {
                        try {
                            isSlim ? SlimSettings.Secure.getIntForUser(
                                    getContentResolver(),
                                    value, UserHandle.USER_CURRENT)
                                    : Settings.Secure.getIntForUser(
                                    getContentResolver(),
                                    value, UserHandle.USER_CURRENT);
                            mSettingType = isSlim ? SLIM_SECURE_INT : SECURE_INT;
                        } catch (SettingNotFoundException b) {
                            try {
                                isSlim ? SlimSettings.System.getLongForUser(
                                        getContentResolver(),
                                        value, UserHandle.USER_CURRENT)
                                        : Settings.System.getLongForUser(
                                        getContentResolver(),
                                        value, UserHandle.USER_CURRENT);
                                mSettingType = isSlim ? SLIM_SYSTEM_LONG : SYSTEM_LONG;
                            } catch (SettingNotFoundException c) {
                                try {
                                    isSlim ? SlimSettings.Secure.getLongForUser(
                                            getContentResolver(),
                                            value, UserHandle.USER_CURRENT)
                                            : Settings.Secure.getLongForUser(
                                            getContentResolver(),
                                            value, UserHandle.USER_CURRENT);
                                    mSettingType = isSlim ? SLIM_SECURE_LONG : SECURE_LONG;
                                } catch (SettingNotFoundException d) {
                                    try {
                                        isSlim ? SlimSettings.System.getFloatForUser(
                                                getContentResolver(),
                                                value, UserHandle.USER_CURRENT)
                                                : Settings.System.getFloatForUser(
                                                getContentResolver(),
                                                value, UserHandle.USER_CURRENT);
                                        mSettingType = isSlim ? SLIM_SYSTEM_FLOAT : SYSTEM_FLOAT;
                                    } catch (SettingNotFoundException e) {
                                        try {
                                            isSlim ? SlimSettings.Secure.getFloatForUser(
                                                    getContentResolver(),
                                                    value, UserHandle.USER_CURRENT)
                                                    : Settings.Secure.getFloatForUser(
                                                    getContentResolver(),
                                                    value, UserHandle.USER_CURRENT);
                                            mSettingType = isSlim ? SLIM_SECURE_FLOAT : SECURE_FLOAT;
    ...
    private void showDialogSetting(int id) {
        switch (id) {
            case DLG_SECRET:
                ...
                .setPositiveButton(R.string.dlg_ok,
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        boolean existsAsSetting = updateSettingType(false /* not slim */, value);
                        boolean existsAsSlimSetting = updateSettingType(true /* slim */, value);
                        boolean resultOk = existsAsSetting || existsAsSlimSetting;

                        // if setting exists in both aosp and slim providers, ask user which one
                        if (existsAsSetting && existsAsSlimSetting) {
                            showDialogSetting(DLG_SECRET_SLIM);
                            updateSettingType(mIsSlimSetting, value);
                        }
            ...
            case DLG_SECRET_SLIM:
                ...
                alertSlim.setTitle(R.string.chamber_slim)
