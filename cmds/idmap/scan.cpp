...
namespace {
    ...
    int idmap_scan(const char *overlay_dir, const char *data_overlay_dir,
            const char *target_package_name, const char *target_apk_path, const char *idmap_dir,
            SortedVector<Overlay>& overlayVector)
    {
        DIR *dir = opendir(overlay_dir);
        DIR *data_dir = opendir(data_overlay_dir);
        if (dir == NULL || data_dir == NULL) {
            return EXIT_FAILURE;
        }
        ...
        while ((dirent = readdir(dir)) != NULL) {
            ...
            if (S_ISDIR(st.st_mode)) {
                ...
                idmap_scan(overlay_apk_path, NULL, target_package_name, target_apk_path, idmap_dir,
                        overlayVector);
            } else {
        ...
        while ((dirent = readdir(data_dir)) != NULL) {
            struct stat st;
            char overlay_apk_path[PATH_MAX + 1];
            snprintf(overlay_apk_path, PATH_MAX, "%s/%s", data_overlay_dir, dirent->d_name);
            if (stat(overlay_apk_path, &st) < 0) {
                continue;
            }
            if (!S_ISREG(st.st_mode) && !S_ISDIR(st.st_mode)) {
            ...
            if (S_ISDIR(st.st_mode)) {
                ...
                idmap_scan(NULL, overlay_apk_path, target_package_name, target_apk_path, idmap_dir,
                        overlayVector);
            } else {
                int priority = parse_apk(overlay_apk_path, target_package_name);
                if (priority < 0) {
        ...
        const char *target_apk_path, const char *idmap_dir)
{
    ...
    SortedVector<Overlay> overlayVector;
    int res = idmap_scan(overlay_dir, data_overlay_dir, target_package_name, target_apk_path, idmap_dir,
            overlayVector);
